...
* 展示头像及经验
* 修改个人信息，路径：`/user/modify_information/modify_information`
* 管理员名单修改（仅管理员可以看到），路径：`//people_selector/people_selector?modify=grant_admin`
* 关于，路径 `/about/about`
...
路径：`/people_selector/people_selector.js`

该表将三个搜索用户多选的场景写在一个页面中，保持主页面逻辑一致和代码复用，同时通过不同的参数（`//people_selector/people_selector?modify=${scene}`）调用不同的 js 中的同名函数，实现不同的功能。
...
`/check_in/check_in.js`

### 初始化用户

路径：`/init_user/init_user`
